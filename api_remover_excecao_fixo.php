...
if (!isset($conn) || !$conn instanceof mysqli) { http_response_code(500); echo json_encode(['success' => false, 'message' => 'Falha DB API RemExcecao.']); exit; }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($bombeiro_id && $data_str) {
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $data_str)) { $response['message'] = 'Formato data inválido.'; http_response_code(400); }
            ...
            if (remover_excecao_fixo($bombeiro_id, $data_str, $conn)) { $response['success'] = true; $response['message'] = 'Exceção removida (Ciclo restaurado).'; }
            else { $response['message'] = 'Erro ao tentar remover exceção.'; http_response_code(500); error_log("API Falha remover exceção ID $bombeiro_id em $data_str"); }
        }
