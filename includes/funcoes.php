<?php
// ARQUIVO ATUALIZADO (includes/funcoes.php)
...
function registrar_excecao_fixo(int $bombeiro_id, string $data, mysqli $conn): bool {
    $sql_bombeiro = "SELECT fixo_ref_data, fixo_ref_dia_ciclo FROM bombeiros WHERE id = ? AND tipo = 'Fixo' AND ativo = 1";
    if (!($stmt_bombeiro = mysqli_prepare($conn, $sql_bombeiro))) {
        error_log("Erro ao preparar a busca do bombeiro fixo: " . mysqli_error($conn));
        return false;
    }
    mysqli_stmt_bind_param($stmt_bombeiro, "i", $bombeiro_id);
    mysqli_stmt_execute($stmt_bombeiro);
    $result = mysqli_stmt_get_result($stmt_bombeiro);
    $bombeiro_data = mysqli_fetch_assoc($result);
    mysqli_stmt_close($stmt_bombeiro);

    if (!$bombeiro_data) {
        error_log("Tentativa de registrar exceção para bombeiro não-fixo ou inativo ID: $bombeiro_id");
        return false;
    }
    $dia_ciclo = calcular_dia_ciclo_fixo($bombeiro_data, $data);

    if ($dia_ciclo !== 1) {
        error_log("Tentativa de registrar exceção para bombeiro ID $bombeiro_id na data $data, mas não era seu dia de serviço (dia do ciclo: $dia_ciclo).");
        return false;
    }
    $sql_insert = "INSERT IGNORE INTO excecoes_ciclo_fixo (bombeiro_id, data) VALUES (?, ?)";
    if ($stmt_insert = mysqli_prepare($conn, $sql_insert)) {
        mysqli_stmt_bind_param($stmt_insert, "is", $bombeiro_id, $data);
        $success = mysqli_stmt_execute($stmt_insert);
        if (!$success) {
            error_log("registrar_excecao_fixo: Erro ao executar o INSERT: " . mysqli_stmt_error($stmt_insert));
        }
...
function get_turno_icon(?string $turno, bool $is_fixo_cycle = false, bool $has_excecao = false): string {
    if ($is_fixo_cycle) {
        return $has_excecao ? '<span class="turno-icon fixo-excecao" title="Fixo Removido (Exceção)">🚫</span>' : '<span class="turno-icon fixo-ciclo" title="Fixo - Ciclo 24h">⏰</span>';
    }
    switch ($turno) {
...
// ===== FUNÇÃO registrar_plantao CORRIGIDA =====
function registrar_plantao(int $bombeiro_id, string $data, string $turno, mysqli $conn): array {
    // Primeiro, busca o tipo do bombeiro que está sendo adicionado.
    $sql_tipo = "SELECT tipo FROM bombeiros WHERE id = ?";
    ...
    $bombeiro = mysqli_fetch_assoc($result_tipo);
    mysqli_stmt_close($stmt_tipo);

    if (!$bombeiro) {
        return ['success' => false, 'message' => 'Bombeiro não encontrado.'];
    }
    $tipo_bombeiro = $bombeiro['tipo'];

    // Se for um BC, aplicamos a lógica de validação de vagas de BC.
    if ($tipo_bombeiro == 'BC') {
        $vagas_bc_d = 1;
        $vagas_bc_n = 1;

        $sql_vagas = "SELECT p.turno FROM plantoes p JOIN bombeiros b ON p.bombeiro_id = b.id WHERE p.data = ? AND b.tipo = 'BC'";
        ...
        while ($plantao_existente = mysqli_fetch_assoc($result_vagas)) {
            ...
            elseif ($plantao_existente['turno'] == 'I') {
                $vagas_bc_d--;
                $vagas_bc_n--;
            }
        }
        ...
        if ($turno == 'D' && $vagas_bc_d <= 0) return ['success' => false, 'message' => 'Não há mais vagas de BC para o turno diurno.'];
        if ($turno == 'N' && $vagas_bc_n <= 0) return ['success' => false, 'message' => 'Não há mais vagas de BC para o turno noturno.'];
        if ($turno == 'I' && ($vagas_bc_d <= 0 || $vagas_bc_n <= 0)) return ['success' => false, 'message' => 'Não há vagas de BC para o plantão integral.'];
    }
    // Se for um Fixo (fazendo uma troca), a lógica de vagas de BC não se aplica.
    // A validação de duplicados abaixo já previne erros.
    ...
    if ($stmt_insert = mysqli_prepare($conn, $sql_insert)) {
        ...
        if (mysqli_stmt_execute($stmt_insert)) {
            mysqli_stmt_close($stmt_insert);
            return ['success' => true, 'message' => 'Plantão registrado com sucesso!'];
        } else {
            if (mysqli_errno($conn) == 1062) {
                 return ['success' => false, 'message' => 'Este bombeiro já está escalado para este dia/turno.'];
            }
            error_log("Erro ao registrar plantão: " . mysqli_stmt_error($stmt_insert));
            mysqli_stmt_close($stmt_insert);
            return ['success' => false, 'message' => 'Erro no banco de dados ao tentar registrar o plantão.'];
        }
    }
    return ['success' => false, 'message' => 'Erro interno ao preparar a query de inserção.'];
}
